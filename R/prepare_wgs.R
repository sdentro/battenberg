...
#' @param min_normal_depth Parameter to exclude SNPs below a certain depth in the matched normal sample (default NA)
#' @author dw9, sd11
#' @export
generate.impute.input.wgs = function(chrom, tumour.allele.counts.file, normal.allele.counts.file, output.file, imputeinfofile, is.male, problemLociFile=NA, useLociFile=NA, heterozygousFilter=0.1, min_normal_depth=NA) {
  ...
  if (!is.na(min_normal_depth)) {
    snps_coverage = normal_snp_data[,7] >= min_normal_depth
    snp_data = snp_data[snps_coverage,]
    normal_snp_data = normal_snp_data[snps_coverage,]
    known_SNPs = known_SNPs[known_SNPs$position %in% snp_data[,2], ]
  }
